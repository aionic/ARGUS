...
def fetch_model_prompt_and_schema(dataset_type, force_refresh=False):
    ...
    max_pages_per_chunk = datasets_config[dataset_type].get(
        "max_pages_per_chunk", 10
    )  # Default to 10 for backward compatibility

    # Get processing options with defaults
    processing_options = datasets_config[dataset_type].get(
        "processing_options",
...
def run_gpt_extraction(
    ...
    try:
        # Debug logging
        logging.info("GPT Extraction Input Debug:")
        ...
        logging.info(
            f"Parsed data keys: {list(extracted_data.keys()) if isinstance(extracted_data, dict) else 'Not a dict'}"
        )
