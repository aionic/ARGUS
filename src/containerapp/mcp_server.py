...
from ai_ocr.agents import run_chat, text_content, user_message
from ai_ocr.process import fetch_model_prompt_and_schema
from dependencies import get_blob_service_client, get_conf_container, get_data_container
...
- Summarize or analyze document content""",
        inputSchema={
            "type": "object",
            "properties": {
                "document_id": {"type": "string", "description": "The unique identifier of the document to chat about"},
                "message": {"type": "string", "description": "Your question or message about the document"},
                "chat_history": {
                    "type": "array",
...
- See the system prompt used for GPT extraction""",
        inputSchema={
            "type": "object",
            "properties": {
                "dataset_name": {"type": "string", "description": "The name of the dataset to get configuration for"}
            },
...
- Access the key information extracted from a document""",
        inputSchema={
            "type": "object",
            "properties": {"document_id": {"type": "string", "description": "The unique identifier of the document"}},
            "required": ["document_id"],
        },
    ),
    Tool(
...
- Locate specific documents in the system""",
        inputSchema={
            ...
            "properties": {
                ...
                "dataset": {"type": "string", "description": "Optional dataset to search within"},
            },
            "required": ["query"],
        },
    ),
...
async def _handle_list_documents(arguments: dict) -> list[TextContent]:
    ...
    try:
        ...
        result = {"total_count": len(documents), "dataset_filter": dataset, "documents": documents}
...
async def _handle_process_document_url(arguments: dict) -> list[TextContent]:
    ...
    try:
        ...
        # Create event data
        event_data = {"url": blob_url, "dataset": dataset}
...
async def _handle_get_extraction(arguments: dict) -> list[TextContent]:
    ...
    try:
        ...
        result = {"document_id": document_id, "filename": item.get("file_name"), "extraction": extraction}
...
async def _handle_create_dataset(arguments: dict) -> list[TextContent]:
    ...
    if not re.match(r"^[a-zA-Z0-9-]+$", dataset_name):
        return [
            TextContent(type="text", text="Error: dataset_name must contain only alphanumeric characters and hyphens")
        ]
    ...
    try:
        ...
        if dataset_name in config_item["datasets"]:
            return [
                TextContent(
                    type="text", text=f"Error: Dataset '{dataset_name}' already exists. Choose a different name."
                )
            ]
