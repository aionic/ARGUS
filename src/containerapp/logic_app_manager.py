...
class LogicAppManager:
    ...
    def __init__(self):
        ...
        if not all([self.subscription_id, self.resource_group_name, self.logic_app_name]):
            logger.warning(
                "Logic App management requires AZURE_SUBSCRIPTION_ID, AZURE_RESOURCE_GROUP_NAME, and LOGIC_APP_NAME environment variables"
            )
            self.enabled = False
        else:
            self.enabled = True
    ...
    async def get_concurrency_settings(self) -> Dict[str, Any]:
        ...
        try:
            ...
            workflow = logic_client.workflows.get(
                resource_group_name=self.resource_group_name, workflow_name=self.logic_app_name
            )
    ...
    async def update_action_concurrency_settings(self, max_runs: int) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(
                f"Successfully updated Logic App {self.logic_app_name} concurrency: trigger and {updated_actions} actions to {max_runs}"
            )
