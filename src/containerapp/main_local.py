...
async def health_check():
    ...
    return HealthResponse(status="healthy", timestamp=datetime.now().isoformat(), version="1.0.0-local")
...
async def upload_file(file: UploadFile = File(...), dataset_name: str = "default-dataset"):
    ...
    document = DocumentModel(
        ...
        extracted_data={"ocr_output": "", "gpt_output": {}, "gpt_evaluation": {}, "gpt_summary": ""},
    )
...
async def process_document(doc_id: str, background_tasks: BackgroundTasks):
    ...
    return {"message": "Document processing started", "id": doc_id, "status": "processing"}
...
async def mock_process_document(doc_id: str):
    ...
    document.extracted_data["gpt_evaluation"] = {"confidence_score": 0.95, "quality_score": 0.88}
...
async def get_config():
    ...
    return {
        ...
        "limits": {"max_file_size_mb": 50, "max_pages": 100},
    }
...
async def update_configuration(config_data: dict):
    ...
    return {"message": "Configuration updated successfully (local development mode)", "config": config_data}
...
if __name__ == "__main__":
    uvicorn.run("main_local:app", host="0.0.0.0", port=8000, reload=True, log_level="info")
